...
/* Formtreue Vereinfachung (Douglas–Peucker): Punkte bleiben, wo die Route abknickt,
   auf Geraden fallen sie weg. So passt auch eine lange Fahrt in eine Anfrage, ohne
   dass die Sehnen Kurven schneiden – die Toleranz wächst nur so weit wie nötig. */
function limitsSimplify(pts, tolM) {
  if (pts.length <= 2) return pts.slice();
  const cosLat = Math.cos(pts[0][0] * Math.PI / 180), mx = 111320 * cosLat, my = 110540;
  const keep = new Uint8Array(pts.length); keep[0] = keep[pts.length - 1] = 1;
  const stack = [[0, pts.length - 1]];
  while (stack.length) {
    const [a, b] = stack.pop();
    if (b - a < 2) continue;
    const ax = pts[a][1] * mx, ay = pts[a][0] * my, bx = pts[b][1] * mx, by = pts[b][0] * my;
    const vx = bx - ax, vy = by - ay, len2 = vx * vx + vy * vy;
    let worst = -1, worstD = tolM;
    for (let i = a + 1; i < b; i++) {
      const px = pts[i][1] * mx - ax, py = pts[i][0] * my - ay;
      let d;
      if (len2 === 0) d = Math.hypot(px, py);
      else { const t = Math.max(0, Math.min(1, (px * vx + py * vy) / len2)); d = Math.hypot(px - vx * t, py - vy * t); }
...
function limitsThinTrack(tr, startTolM, maxPts) {
  maxPts = maxPts || 650;
  const raw = [];
  ...
  for (let i = 0; i < tr.n; i++) {
    ...
    if (raw.length && haversine(lastLat, lastLon, la, lo) < 5) continue;   // GPS-Zittern im Stand
    raw.push([la, lo]); lastLat = la; lastLon = lo;
  }
  let tol = startTolM || 8, pts = raw;
  for (let pass = 0; pass < 12; pass++) {
    pts = limitsSimplify(raw, tol);
    if (pts.length <= maxPts) break;
    tol *= 1.5;
  }
  pts.tolM = Math.ceil(tol);
  return pts;
}
...
async function fetchLimitWays(tr, onStatus, onPartial) {
  ...
  for (let c = 0; c < chunks.length; c++) {
    ...
    const part = await fetchOverpassChunk(chunks[c], (text) => { if (onStatus) onStatus(label + text, frac); }, radius);
...
async function fetchOverpassChunk(pts, onStatus, radius) {
  const q = limitsOverpassQuery(pts, radius || 25);
  let lastErr = null;
