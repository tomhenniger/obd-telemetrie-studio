...
const METRICS = [
  /* --- Motor --- */
  { id:'rpm', label:'Motordrehzahl', short:'Drehzahl', u:'rpm', d:0, g:'motor', p:10, c:'#e2745f',
  ...
  { id:'load_abs', label:'Absolute Motorlast', short:'Last (abs.)', u:'%', d:1, g:'motor', p:9, c:'#cf905a',
    a:[/^absolute\s*load(\s*value)?$/, /^absolute\s*engine\s*load/, /^abs.*last/] },
  { id:'load_calc', label:'Berechnete Motorlast', short:'Last (ber.)', u:'%', d:1, g:'motor', p:8, c:'#d09c66',
    a:[/^(calculated\s*)?engine\s*load(\s*value)?$/, /^calculated\s*load/, /^motorlast/, /^engine\s*load$/] },
  { id:'timing', label:'Zündwinkel', short:'Zündwinkel', u:'°', d:1, g:'motor', p:9, c:'#9c86c8',
    a:[/^timing\s*advance/, /^ignition\s*(timing|advance|angle)/, /^zündwinkel/, /^zuendwinkel/, /^spark\s*advance/] },
  { id:'knock_retard', label:'Klopfrücknahme', u:'°', d:1, g:'motor', p:8, c:'#d25a52',
    a:[/knock\s*(retard|correction)/, /klopf(rücknahme|regelung)/, /ignition\s*retard/] },
  { id:'throttle', label:'Drosselklappe', short:'Drosselklappe', u:'%', d:1, g:'motor', p:8, c:'#56a4ab',
    a:[/^throttle\s*position/, /^absolute\s*throttle/, /^drosselklappe/, /^rel(ative)?\s*throttle/, /^commanded\s*throttle/] },
  { id:'pedal', label:'Fahrpedalstellung', short:'Fahrpedal', u:'%', d:1, g:'motor', p:9, c:'#66aeb2',
  ...
  { id:'power', label:'Motorleistung (aus Verbrauch)', short:'Leistung (App)', u:'PS', d:0, g:'motor', p:9, c:'#d67c9c',
    a:[/^instant(aneous)?\s*engine\s*power/, /^engine\s*power/, /^power\s*\(/, /^power$/, /^leistung/, /^motorleistung/] },
  { id:'torque', label:'Drehmoment', u:'Nm', d:0, g:'motor', p:8, c:'#b48ac6',
    a:[/^(engine\s*)?torque/, /^drehmoment/] },

  /* --- Aufladung --- */
  { id:'boost', label:'Ladedruck', short:'Ladedruck', u:'bar', d:2, g:'boost', p:10, c:'#6b9cc0', pressure:true,
  ...
  { id:'boost_target', label:'Ladedruck Soll', u:'bar', d:2, g:'boost', p:8, c:'#88aac4', pressure:true,
    a:[/boost.*(desired|target|commanded|soll)/, /(desired|commanded).*boost/] },
  { id:'map', label:'Saugrohrdruck (absolut)', short:'MAP', u:'kPa', d:0, g:'boost', p:9, c:'#7a86bd',
    a:[/^intake\s*manifold\s*(absolute\s*)?pressure/, /^manifold\s*(absolute\s*)?pressure/, /^map$/, /^saugrohrdruck/] },
  { id:'baro', label:'Umgebungsdruck', short:'Luftdruck', u:'kPa', d:1, g:'boost', p:7, c:'#959cc0',
    a:[/^barometric\s*pressure/, /^ambient\s*(air\s*)?pressure/, /^luftdruck/, /^umgebungsdruck/] },
  { id:'maf', label:'Luftmasse', short:'MAF', u:'g/s', d:1, g:'boost', p:8, c:'#6fbf7d',
  ...
  /* --- Temperaturen --- */
  { id:'coolant', label:'Kühlmitteltemperatur', short:'Kühlmittel', u:'°C', d:0, g:'temp', p:10, c:'#e06b62',
    a:[/^engine\s*coolant\s*temp/, /^coolant\s*temp/, /^kühlmittel/, /^kuehlmittel/, /^ect$/] },
  { id:'oil_temp', label:'Öltemperatur', short:'Öl', u:'°C', d:0, g:'temp', p:9, c:'#c98c46',
    a:[/^(engine\s*)?oil\s*temp/, /^öltemp/, /^oeltemp/] },
  { id:'iat', label:'Ansauglufttemperatur', short:'Ansaugluft', u:'°C', d:0, g:'temp', p:9, c:'#7aa4c4',
    a:[/^intake\s*air\s*temp/, /^iat$/, /^ansaugluft/, /^air\s*intake\s*temp/] },
  { id:'cac_b1', label:'Ladelufttemperatur Bank 1', short:'LLK B1', u:'°C', d:0, g:'temp', p:9, c:'#55aca0',
    a:[/charge\s*air\s*cooler\s*temp.*bank\s*1/, /ladeluft.*bank\s*1/, /^cact.*b1/] },
  { id:'cac_b2', label:'Ladelufttemperatur Bank 2', short:'LLK B2', u:'°C', d:0, g:'temp', p:9, c:'#3f9c8f',
    a:[/charge\s*air\s*cooler\s*temp.*bank\s*2/, /ladeluft.*bank\s*2/, /^cact.*b2/] },
  { id:'ambient', label:'Außentemperatur', short:'Außenluft', u:'°C', d:0, g:'temp', p:8, c:'#80b0c6',
  ...
  /* --- Gemisch / Abgas --- */
  { id:'ltft_b1', label:'Langzeit-Gemischkorrektur Bank 1', short:'LTFT B1', u:'%', d:2, g:'fuel', p:10, c:'#cfa858',
    a:[/^long\s*term\s*fuel\s*%?\s*trim.*bank\s*1/, /^ltft.*(b1|bank\s*1)/, /langzeit.*bank\s*1/] },
  { id:'ltft_b2', label:'Langzeit-Gemischkorrektur Bank 2', short:'LTFT B2', u:'%', d:2, g:'fuel', p:10, c:'#c99b45',
    a:[/^long\s*term\s*fuel\s*%?\s*trim.*bank\s*2/, /^ltft.*(b2|bank\s*2)/, /langzeit.*bank\s*2/] },
  { id:'stft_b1', label:'Kurzzeit-Gemischkorrektur Bank 1', short:'STFT B1', u:'%', d:2, g:'fuel', p:10, c:'#b7bf5c',
    a:[/^short\s*term\s*fuel\s*%?\s*trim.*bank\s*1/, /^stft.*(b1|bank\s*1)/, /kurzzeit.*bank\s*1/] },
  { id:'stft_b2', label:'Kurzzeit-Gemischkorrektur Bank 2', short:'STFT B2', u:'%', d:2, g:'fuel', p:10, c:'#a5a44a',
  ...
  /* --- Verbrauch --- */
  { id:'fuel_rate', label:'Momentaner Kraftstofffluss', short:'Fluss', u:'L/h', d:2, g:'cons', p:10, c:'#a8887a',
    a:[/^(calculated\s*)?instant.*fuel\s*rate/, /^fuel\s*(flow\s*)?rate/, /^engine\s*fuel\s*rate/, /kraftstofffluss/] },
  { id:'cons_inst', label:'Momentanverbrauch', short:'Verbrauch (App)', u:'L/100km', d:1, g:'cons', p:10, c:'#b39a8e',
  ...
  /* --- Fahrdynamik / Strecke --- */
  { id:'speed', label:'Geschwindigkeit (OBD)', short:'Tempo (OBD)', u:'km/h', d:0, g:'drive', p:10, c:'#7aa4c4',
    a:[/^vehicle\s*speed$/, /^speed$/, /^geschwindigkeit$/, /^obd\s*speed/] },
  { id:'speed_gps', label:'Geschwindigkeit (GPS)', short:'Tempo GPS', u:'km/h', d:0, g:'drive', p:9, c:'#6ea6c0',
  ...
  { id:'accel', label:'Längsbeschleunigung', short:'Beschl.', u:'g', d:3, g:'drive', p:9, c:'#a2c274',
    a:[/^vehicle\s*acceleration/, /^acceleration$/, /^beschleunigung/, /^g[\s-]?force/] },
  { id:'altitude', label:'Höhe (GPS)', short:'Höhe', u:'m', d:1, g:'drive', p:8, c:'#9fc8a3',
  ...
  /* --- Elektrik --- */
  { id:'batt', label:'Bordspannung', short:'Spannung', u:'V', d:2, g:'misc', p:8, c:'#cdb86a',
