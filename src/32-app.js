...
/* ===== Auswertung teilen ===========================================
   Der Link trägt die Zusammenfassung in sich: Kennzahlen, bewertete Befunde,
   Fahrzeugprofil. Keine Route, keine Rohdaten, kein Server. Wer ihn öffnet,
   sieht dieselbe Zusammenfassung im Browser und kann von dort weiterarbeiten.
   ================================================================== */
async function openShareDialog() {
  if ($('#share-dlg')) return;
  const close = () => { const n = $('#share-dlg'); if (n) n.remove(); document.removeEventListener('keydown', esc); };
  const esc = e => { if (e.key === 'Escape') close(); };
  const body = el('div', { class: 'mdl-b' });
  const dlg = el('div', { class: 'mdl-bd', id: 'share-dlg', onclick: e => { if (e.target === e.currentTarget) close(); } },
    el('div', { class: 'mdl', role: 'dialog', 'aria-modal': 'true', 'aria-label': 'Auswertung teilen' },
      el('div', { class: 'mdl-h' },
        el('h3', {}, 'Auswertung teilen'),
        el('p', {}, 'Der Link enthält die Zusammenfassung selbst – Kennzahlen, bewertete Befunde und das Fahrzeugprofil. Ohne Server, ohne Konto, ohne Route.')),
      body,
      el('div', { class: 'mdl-f' }, el('button', { class: 'btn', type: 'button', onclick: close }, 'Schließen'))));
  document.body.appendChild(dlg);
  document.addEventListener('keydown', esc);
  body.appendChild(el('p', { class: 'dim' }, 'Link wird erzeugt …'));

  if (!App.ds || !App.diag) {
    body.innerHTML = '';
    body.appendChild(noteBox('info', 'Erst eine Fahrt laden',
      'Zum Teilen braucht es eine ausgewertete Aufzeichnung. Lade eine CSV-Datei, dann steht hier der Link.'));
    return;
  }
  let url = '', code = '';
  try {
    code = await encodeShare(shareSummary(App.ds, App.diag, App.profile, App.gears, App.fileName));
    url = shareUrl(code);
  } catch (e) {
    body.innerHTML = '';
    body.appendChild(noteBox('crit', 'Link konnte nicht erzeugt werden', String(e.message || e)));
    return;
  }
  const field = el('input', { class: 'inp', readonly: true, value: url, style: { width: '100%' },
    onclick: e => e.target.select() });
  let qrSvgStr = null;
  try { qrSvgStr = qrSvg(url, { px: 260, quiet: 3, fg: '#000000', bg: '#ffffff' }); }
  catch (e) { qrSvgStr = null; }

  const copyBtn = el('button', { class: 'btn primary', type: 'button', onclick: async () => {
    try { await navigator.clipboard.writeText(url); copyBtn.textContent = 'Kopiert'; setTimeout(() => { copyBtn.innerHTML = ''; copyBtn.append(icon('clip'), 'Link kopieren'); }, 1600); }
    catch (e) { field.select(); }
  } }, icon('clip'), 'Link kopieren');
  const shareBtn = (navigator.share) ? el('button', { class: 'btn', type: 'button', onclick: async () => {
    try { await navigator.share({ title: 'OBD-Auswertung' + (App.profile ? ' · ' + App.profile.name : ''), text: 'Auswertung meiner OBD-Aufzeichnung', url }); }
    catch (e) { /* Abbruch durch den Nutzer ist kein Fehler */ }
  } }, icon('share'), 'Teilen …') : null;
  const qrDl = qrSvgStr ? el('button', { class: 'btn', type: 'button', onclick: () => {
    download('obd-auswertung-qr.svg', 'image/svg+xml', qrSvgStr);
  } }, icon('dl'), 'QR als Bild') : null;

  body.innerHTML = '';
  body.appendChild(el('div', { class: 'share-wrap' },
    qrSvgStr ? el('div', { class: 'share-qr', html: qrSvgStr }) : null,
    el('div', { class: 'share-side' },
      el('div', { class: 'lbl-eng', style: { marginBottom: '6px' } }, 'Link'),
      field,
      el('div', { class: 'chiprow', style: { marginTop: '10px' } }, copyBtn, shareBtn, qrDl),
      el('p', { class: 'dim2', style: { fontSize: '11.5px', marginTop: '10px', lineHeight: '1.5' } },
  ...
  body.appendChild(el('p', { class: 'dim', style: { fontSize: '12.5px', lineHeight: '1.6' } },
    'Fahrzeugprofil, Kennzahlen der Fahrt und ' + ((t.ok || 0) + (t.warn || 0) + (t.crit || 0)) + ' bewertete Befunde. ' +
    'Nicht enthalten: die gefahrene Route, die Zeitreihen und die Rohdatei. Wer die vollständige Auswertung sehen will, braucht die CSV-Datei.'));
}

/* ===== Bericht öffnen und schließen ================================ */
async function openReport(kindKey) {
  const key = kindKey || store.get('reportKind', 'werkstatt');
  store.set('reportKind', key);
  const old = document.getElementById('full-report');
  if (old && !kindKey) return;
  if (old) old.remove();
  document.body.classList.add('report-mode');
  const bar = document.querySelector('.rep-bar') || el('div', { class: 'rep-bar' });
  bar.innerHTML = ''; bar.appendChild(el('span', { class: 'dim' }, 'Bericht wird gesetzt …'));
  if (!bar.isConnected) document.body.appendChild(bar);
  await new Promise(r => setTimeout(r, 30));
  let doc;
  try { doc = await buildReportDoc(key); }
  catch (e) {
    bar.innerHTML = '';
    bar.append(el('span', { class: 'dim' }, 'Bericht fehlgeschlagen: ' + (e.message || e)),
      el('button', { class: 'btn sm', type: 'button', onclick: () => closeReport() }, 'Zurück'));
    console.error(e);
    return;
  }
  document.body.appendChild(doc.node);
  const sel = el('select', { class: 'sel', onchange: e => openReport(e.target.value) },
    Object.keys(REPORT_KINDS).map(k => el('option', { value: k, selected: k === key ? true : null },
      REPORT_KINDS[k].label + ' – ' + REPORT_KINDS[k].hint)));
  bar.innerHTML = '';
  bar.append(
    el('span', { class: 'field' }, el('span', { class: 'dim' }, 'Bericht'), sel),
    el('span', { class: 'dim2' }, doc.pages + ' Seiten · ' + doc.parts + ' Abschnitte'),
...
(function init() {
  ...
  const shrBtn = $('#share'); if (shrBtn) shrBtn.onclick = openShareDialog;
  /* Tastenkürzel: T teilt, B erzeugt den Bericht – nur, wenn nicht gerade getippt wird */
  document.addEventListener('keydown', e => {
    if (e.metaKey || e.ctrlKey || e.altKey) return;
    const tag = (e.target && e.target.tagName) || '';
    if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT') return;
    if (e.key === 't' || e.key === 'T') { e.preventDefault(); openShareDialog(); }
    if (e.key === 'b' || e.key === 'B') { e.preventDefault(); openReport(); }
  });
