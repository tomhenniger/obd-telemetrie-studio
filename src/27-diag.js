  ...
  run(c) {
    ...
    return { status: st, value: pMax, unit: '%', dec: 1, ref: '≥ 55 % bei Volllast', refLo: 55, refHi: 100,
      extra: [['Ruhewert', fmt(pIdle, 1) + ' %'],
              ...
              isFinite(dt) ? ['Abstand beider Maxima', fmt(dt, 1) + ' s'] : null].filter(Boolean),
      text: st === S.OK
   ...
   lieber zurück, als etwas zu behaupten. */
{
  ...
  run(c) {
    ...
    return { status: st, value: med, unit: '°C', dec: 0, ref: '80–120 °C, Spitze ≤ 125 °C', refLo: 80, refHi: 120,
      cond: 'Kühlmittel ≥ 80 °C und Motor läuft',
      extra: [['Maximum', fmt(mx, 0) + ' °C'], isFinite(co) ? ['Kühlmittel dabei', fmt(co, 0) + ' °C'] : null, ['Bewertete Zeit', fmtDur(d)]].filter(Boolean),
      text: st === S.OK
  ...
  run(c) {
    ...
    return { status: st, value: med, unit: '°C', dec: 0, ref: '350–800 °C, über 900 °C nur sekundenweise', refLo: 350, refHi: 800,
      ...
      extra: [['Maximum', fmt(mx, 0) + ' °C'], ['Zeit über 900 °C', fmtDur(hot)],
              isFinite(delta) ? ['Unterschied Bank 1/2', fmt(delta, 0) + ' K'] : null].filter(Boolean),
      text: st === S.OK
...
},
{
  ...
  run(c) {
    ...
    return { status: st, value: isFinite(perL) ? perL : r, unit: isFinite(perL) ? 'g/s·L' : 'r', dec: 2,
      ...
      extra: [isFinite(idleMaf) ? ['Luftmasse im Leerlauf', fmt(idleMaf, 1) + ' g/s'] : null, isFinite(r) ? ['Korrelation zur Last', fmt(r, 3)] : null].filter(Boolean),
      text: st === S.OK
...
},
{
  ...
  run(c) {
    ...
    return { status: st, value: isFinite(pW) ? pW : pI, unit: 'kPa', dec: 0,
      ...
      extra: [isFinite(pI) ? ['Leerlauf', fmt(pI, 0) + ' kPa'] : null, isFinite(pW) ? ['Volllast (p05)', fmt(pW, 0) + ' kPa'] : null, ['Spitze', fmt(s.max, 0) + ' kPa']].filter(Boolean),
      text: st === S.OK
...
},
{
  ...
  run(c) {
    ...
    return { status: st, value: eP, unit: '%', dec: 0, ref: 'Teillast 5–40 %, Volllast ≈ 0 %', refLo: 5, refHi: 40,
      ...
      extra: [['AGR in der Teillast', fmt(eP, 0) + ' %'], isFinite(eW) ? ['AGR unter Volllast (p95)', fmt(eW, 0) + ' %'] : null].filter(Boolean),
      text: st === S.OK
